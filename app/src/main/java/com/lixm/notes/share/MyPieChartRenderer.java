...
public class MyPieChartRenderer extends DataRenderer {
    ...
    public MyPieChartRenderer(MyPieChart chart, ChartAnimator animator,
    ...
    public void drawValues(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = 0; j < entryCount; j++) {
                ...
                int textHeight = calTextHeight(entry);
...
//                            mEntryLabelsPaint.setColor(setLineColor);
                              mEntryLabelsPaint.setColor(dataSet.getValueTextColor(j));
                        }
...
//                                c,
//                                icon,
//                                transformedAngle % 360.0 >= 90.0 && transformedAngle % 360.0 <= 270.0 ? (int) (pt2x - Utils.convertDpToPixel(50)-icon.getIntrinsicWidth() / 2 - Utils.convertDpToPixel(5)) : (int) (pt2x + Utils.convertDpToPixel(50) + icon.getIntrinsicWidth()/2 + Utils.convertDpToPixel(5)),
//                                (int) pt2y,
...
//                        drawValue(c,formattedValue,labelPtx,labelPty + lineHeight ,
//                                /**setLineColor这里是重点,源码设置是:dataSet.getValueTextColor(j) 改成自己的颜色 修改源码,百分比的值的颜色*/
//                                setLineColor);

                        drawValue(c,formattedValue, labelPtx,
                                labelPty  + lineHeight,
                        ...
                        drawValue(c, formattedValue, labelPtx, labelPty + lineHeight / 2.f, valueColor);
                    }

                    if (dataSet.getValueLineColor() != ColorTemplate.COLOR_NONE) {
                        c.drawLine(pt0x, pt0y, pt1x, pt1y, mValueLinePaint);
                        c.drawLine(pt1x, pt1y, labelPtx, labelPty  + lineHeight, mValueLinePaint);
                    }
                }
...
//                }
    ...
    private int calTextHeight(PieEntry entry) {
        int textHeight = 0;
        if(!TextUtils.isEmpty(entry.getLabel())) {
            Rect rect = new Rect();
            getPaintEntryLabels().getTextBounds(entry.getLabel(), 0, entry.getLabel().length(), rect);
            int entryLabelsTextHeight = rect.height();
            textHeight += entryLabelsTextHeight;
            Log.d("xxx","xxx1:"+entryLabelsTextHeight);
        }
        if(!TextUtils.isEmpty(entry.getValue()+"")){
            Rect rect = new Rect();
            getPaintEntryLabels().getTextBounds(entry.getValue()+"", 0, (entry.getValue()+"").length(), rect);
            int valueTextHeight = rect.height();
            textHeight += valueTextHeight;
            Log.d("xxx","xxx2:"+valueTextHeight);
        }
        return textHeight;
    }
